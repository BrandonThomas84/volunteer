...
class calendar {
	...
	public function __construct($calType = null) {
		...
		//get first position NEW TESTING
		$earliest_year = false;
		$earliest_month = false;
		$earliest_day = false;
		$first_position = get_first_position();
		if ($first_position) {
			$earliest_year = date('Y', $first_position['date_start']);
			$earliest_month = date('m', $first_position['date_start']);
			$earliest_day = date('d', $first_position['date_start']);
		}
